/**
 * The AMB resumable is an extension of http://github.com/23/resumable.js
 */
...
var Resumable = function(opts){
  ...
  var $ = this;
  $.files = [];
  $.currentFilesInUploading = 0;
  $.defaults = {
    chunkSize:1*1024*1024,
    forceChunkSize:false,
    simultaneousChunksUploads:3,
    simultaneousFilesInUploads:1, /** AMB introduced */
    ...
    permanentErrors:[401, 415, 500, 501], /** AMB introduced 401 Unauthorized */
    maxFiles:undefined,
  ...
  /**
   * AMB just allow authenticated user/application for successful uploads
   * @param token
  */
  $.userSessionToken = function(token){
    var headers = $.getOpt('headers');
  ...
  var $h = {
    ...
    each: function(o,callback){
      ...
      /**
       * AMB implementation for producing a JS guid
       * @param file
       * @returns {string}
       */
    generateUniqueIdentifier:function(file){
      ...
      /**
       * AMB implementation for checksum. It uses the https://code.google.com/p/crypto-js/
       * @param reader
       * @returns md5 hash String
       */
    md5:function(reader){
        var blobBinaryString = reader.result;
        var int8View = new Int8Array(blobBinaryString);
        var words = [], u8arr = int8View, len = u8arr.length;

        for (var i = 0; i < len; i++) { words[i >>> 2] |= (u8arr[i] & 0xff) << (24 - (i % 4) * 8); }

        var wordarray = CryptoJS.lib.WordArray.create(words, len);
  ...
  function ResumableChunk(resumableObj, fileObj, offset, callback){
    ...
    $.send = function(){
      ...
      $.xhr.addEventListener("error", doneHandler, false);
        ...
        /**
         *
         * AMB has to deal with checksum for it chunk created
         */
        var reader = new FileReader();
        reader.onloadend = function() {

            var chunkMD5CheckSum = $h.md5(reader);
            ...
            var query = {
                ...
                keyAmbCheckSum: chunkMD5CheckSum /** AMB introduced */
            };
            // Mix in custom data
            var customQuery = $.getOpt('query');
            if(typeof customQuery == "function") customQuery = customQuery($.fileObj, $);
            ...
            // Reader may have been "cancelled", so verify it exists first;
            if ($.xhr) {
  ...
  $.uploadNextChunk = function(){
    ...
    if(!outstanding) {
      ...
      $.decreaseNumberOfFilesInUploading();
      $.fire('complete');
    }
    return(false);
  };
  ...
  /**
    Introduced by AMB. This function controls the possible
    number of uploading files simultaneously
     */
  $.maxOfSimultaneousFilesUploading = function(){
    if($.currentFilesInUploading == $.getOpt('simultaneousFilesInUploads')) true;
    else{
        $.currentFilesInUploading++;
        return false;
    }
  }
    /**
     * Introduced by AMB. This function helps to coordinate the
     * number of files uploading at the same time
      */
  $.decreaseNumberOfFilesInUploading = function(){
     if($.currentFilesInUploading <= 1) $.currentFilesInUploading = 0;
     else $.currentFilesInUploading--;
  }
  $.upload = function(){
    /** @AMB Allow multiple uploads */
    if($.maxOfSimultaneousFilesUploading()) return;
    // Kick off the queue
    $.fire('uploadStart');
    for (var num=1; num<=$.getOpt('simultaneousChunksUploads'); num++) {
      $.uploadNextChunk();
    }
  };
  $.pause = function(){
